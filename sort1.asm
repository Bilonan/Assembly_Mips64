...
# Buble-sort algorythm:  $t0 - address of the first number, $t1 - loop counter, $a0, $a1, $t2 - temporary values
 ...
 L_sort1:                     # begin sort loop: compare two numbers:
   ...
   ble $a0,$a1,NO_sort2       # if the numbers are sorted, then continue sorting.
        ...
        move $a2,$ra           # save link register before procedure
        jal Proc_swap_elements # call swap procedure 
        move $ra,$a2           # restore link register after procedure
...
Proc_swap_elements:           # swap the first and second number
      sw $a1,0($t0)           # first number = a1
      sw $a0,4($t0)           # second number = a0
      
   beq $t0, $t2, EX_NO_sort2  # if the first number is on the beginning of array, then continue sorting,

      subi $t0, $t0, 4        # else take the previous element
      addi $t1, $t1, 1        # and increment loop counter
      jr   $ra                # return

  EX_NO_sort2:                # exit from procedure:
      move $ra,$a2            # restore link register
  j NO_sort2              # continue from label "NO_sort2" in parent procedure
#-----------------------------------------------------------------
